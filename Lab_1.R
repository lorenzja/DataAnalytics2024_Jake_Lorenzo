...
x_APO <- seq(min(APO.new, na.rm=TRUE), max(APO.new, na.rm=TRUE), 1)
q_APO <- dnorm(x_APO, mean = mean(APO.new, na.rm=TRUE), sd = sd(APO.new, na.rm=TRUE))
lines(x_APO, q_APO)
...
x_WRS <- seq(min(WRS.new, na.rm=TRUE), max(WRS.new, na.rm=TRUE), 1)
q_WRS <- dnorm(x_WRS, mean = mean(WRS.new, na.rm=TRUE), sd = sd(WRS.new, na.rm=TRUE))
lines(x_WRS, q_WRS)
